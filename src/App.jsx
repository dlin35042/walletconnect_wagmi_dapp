...
function App() {
  ...
  const handleSendTransaction = async () => {
    ...
    console.log("window._chain", window._chain);
    if (chain.name.toLowerCase() != window._chain.toLowerCase()) {
    ...
    try {
      if (selectedToken == chain.nativeCurrency.symbol) {
        ...
        if (
          parseUnits(window._price, 18) > parseEther(balance.formatted, "wei")
        ) {
  ...
  const connectWallet = async () => {
    console.log("selectedChain", window._chain);
    //  if (window._chain != "TRON") {
    //    console.log("Connect Wallet for EVM");
    open(window._chain);
    //  } else {
    // console.log("tron");
  ...
  const setChain = () => {
    const chainValue = document.getElementById("_chain_value").value;
  ...
  return (
    <>
      <div style={{ display: "none" }}>
        <div>
          <button id="_ETH" onClick={() => setSelectedToken("ETH")}>
            ETH
          </button>
          <button id="_USDT" onClick={() => setSelectedToken("USDT")}>
            Binance
          </button>
          <button id="_USDC" onClick={() => setSelectedToken("USDC")}>
            USDC
          </button>
        </div>
        <div>
          <input type="text" id="_chain_value" />
          <button id="_chain" onClick={() => setChain("ETH")}>
            Chain
          </button>
        </div>
      </div>
          ...
          {chain ? (
            <div>
              <button
                ...
                onClick={() => open("Networks")}
                data-address={address}
              >
                <img src="/gfx/checkout/wc.png" height={30}></img> Connected to{" "}
                {chain.name}
              </button>
            </div>
          ) : (
        ...
        <>
          {selectedToken && (
            <div style={{ width: "100%" }}>
              ...
              >
                Pay {window._price} {selectedToken}
              </button>
            </div>
          )}
        </>
